...
public class AuthorizationServiceImpl implements AuthorizationService {
    ...
    public Map<Long, AuthorizationInfoDTO> authorize(String username, Long[] tenantIds) {
        ...
        if (map.size() < 1) {
            ...
            //构建默认权限对象
            AuthorizationInfoDTO info = new AuthorizationInfoDTO();

            //构建权限
            Map<String,List<String>>  rolePrivilegeMap = new HashMap<>(1);

            ArrayList<String> privileages = new ArrayList<>();
            privileages.add("xc_base_privileage");
            rolePrivilegeMap.put("r_xc_com", privileages);

            info.setRolePrivilegeMap(rolePrivilegeMap);

            //给租户id构建默认权限
            for (Long tenantId : tenantIds) {
                map.put(tenantId,info);
            }
        }
